...
export default class Controls {
  ...
  bind() {
    ...
    pointerEvents.forEach((eventName) => {
      const options = eventName === "touchstart" ? { passive: true } : false; // Agregar passive: true para touchstart
      this.controlsLeft.forEach((el) =>
        el.addEventListener(eventName, this.onNavigateLeftClicked, options)
      );
      this.controlsRight.forEach((el) =>
        el.addEventListener(eventName, this.onNavigateRightClicked, options)
      );
      this.controlsUp.forEach((el) =>
        el.addEventListener(eventName, this.onNavigateUpClicked, options)
      );
      this.controlsDown.forEach((el) =>
        el.addEventListener(eventName, this.onNavigateDownClicked, options)
      );
      this.controlsPrev.forEach((el) =>
        el.addEventListener(eventName, this.onNavigatePrevClicked, options)
      );
      this.controlsNext.forEach((el) =>
        el.addEventListener(eventName, this.onNavigateNextClicked, options)
      );
      this.controlsFullscreen.forEach((el) =>
        el.addEventListener(eventName, this.onEnterFullscreen, options)
      );
    });
  }
